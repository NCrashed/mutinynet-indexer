...
use crate::{db::loaders::FieldDecode, vault::{VaultId, VaultTx}};
...
impl DatabaseVaultAdvance for Connection {
    fn range_history_all(
        ...
        let rows = statement.query_map(named_params! {":start": start.unwrap_or(0), ":end": end.unwrap_or(u32::MAX)}, load_vault_meta).map_err(Error::ExecuteQuery)?;
        Ok(rows.map(|row| row.map_err(Error::FetchRow)).collect::<Result<Vec<_>, Error>>()?)
    }
}
